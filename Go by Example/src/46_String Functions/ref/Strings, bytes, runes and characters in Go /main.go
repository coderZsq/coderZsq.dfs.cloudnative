...
func init() {
	...
	// `⌘`
	// places the UTF-8 encoding of the symbol ⌘ into the source text. When we print out the hexadecimal bytes, we’re just dumping the data the editor placed in the file.
...
func init() {
	...
	// U+65E5 '日' start at byte position 0
	// U+672C '本' start at byte position 3
	// U+8A9E '語' start at byte position 6
