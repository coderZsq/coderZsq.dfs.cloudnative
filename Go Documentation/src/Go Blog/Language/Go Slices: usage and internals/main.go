...
func slices() {
	...
	*/
	...
	b := []byte{'g', 'o', 'l', 'a', 'n', 'g'}
	fmt.Println(b[1:4]) // b[1:4] == []byte{'o', 'l', 'a'}, sharing the same storage as b
	// The start and end indices of a slice expression are optional; they default to zero and the slice’s length respectively:
	fmt.Println(b[:2]) // b[:2] == []byte{'g', 'o'}
	fmt.Println(b[2:]) // b[2:] == []byte{'l', 'a', 'n', 'g'}
	fmt.Println(b[:])  // b[:] == b
...
func slice_internals() {
	...
	*/
	...
	d := []byte{'r', 'o', 'a', 'd'}
	e := d[2:]
	fmt.Println(e) // e == []byte{'a', 'd'}
	e[1] = 'm'
	fmt.Println(e) // e == []byte{'a', 'm'}
	fmt.Println(d) // d == []byte{'r', 'o', 'a', 'm'}
...
func growing_slices() {
	...
	*/
	...
	p = AppendBytes(p, 7, 11, 13)
	fmt.Println(p, len(p), cap(p)) // p == []byte{2, 3, 5, 7, 11, 13}
	...
	a := make([]int, 1)
	fmt.Println(a) // a == []int{0}
	a = append(a, 1, 2, 3)
	fmt.Println(a) // a == []int{0, 1, 2, 3}
	// To append one slice to another, use ... to expand the second argument to a list of arguments.
	a2 := []string{"John", "Paul"}
	b2 := []string{"George", "Ringo", "Pete"}
	a2 = append(a2, b2...) // equivalent to "append(a, b[0], b[1], b[2])"
	fmt.Println(a2)        // a == []string{"John", "Paul", "George", "Ringo", "Pete"}
	// Since the zero value of a slice (nil) acts like a zero-length slice, you can declare a slice variable and then append to it in a loop:
	s2 := []int{1, 2, 3, 4, 5}
	s2 = Filter(s2, func(i int) bool {
		return i%2 == 0
	})
	fmt.Println(s2)
}
